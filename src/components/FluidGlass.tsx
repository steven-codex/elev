...
export default function FluidGlass({
  ...
  return (
    <div className={`w-full h-full relative ${className}`} style={{ backgroundColor, ...style }}>
      <Canvas
...
const ModeWrapper = memo(function ModeWrapper({
  ...
  lockToBottom = false,
  followPointer = true,
  modeProps = {} as Record<string, any>,
  backgroundColor = 'transparent',
  ...props
}: any) {
      ...
      {createPortal(
        <>
          <mesh position={[0, 0, -5]} scale={[vp.width * 2, vp.height * 2, 1]}>
            <planeGeometry />
            <meshBasicMaterial color={backgroundColor === 'transparent' ? '#120F17' : backgroundColor} toneMapped={false} />
          </mesh>
          {children}
        </>,
        scene
...
function NavItems({ items, textColor }: { items: Array<{ label: string; link: string }>; textColor: string }) {
